...
void drv_FlashInit( void )
{
    ...
    FLASH_TYPE = drv_FLASH_ReadID();	            //读取FLASH ID.
}
...
void drv_FLASH_Read(u8* pBuffer,u32 ReadAddr,u16 NumByteToRead)   
{ 
 	u16 i;   										    
	FLASH_CS=0;                            	//使能器件   
...
void drv_FLASH_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
 	u16 i;  
    drv_FLASH_Write_Enable();                  //SET WEL 
	FLASH_CS=0;                            //使能器件   
...
{ 			 		 
	...
	while(1)
	{	   
		drv_FLASH_Write_Page(pBuffer,WriteAddr,pageremain);
...
#endif
void drv_FLASH_Write(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)   
{ 
	u32 secpos;
	u16 secoff;
	u16 secremain;	   
	...
	while(1) 
	{	
		drv_FLASH_Read(FLASH_BUF,secpos*4096,4096);//读出整个扇区的内容
		...
		if(i<secremain)//需要擦除
		{
			drv_FLASH_Erase_Sector(secpos);	//擦除这个扇区
			for(i=0;i<secremain;i++)	  	//复制
			{
				FLASH_BUF[i+secoff]=pBuffer[i];	  
			}
			drv_FLASH_Write_NoCheck(FLASH_BUF,secpos*4096,4096);//写入整个扇区  

		}else drv_FLASH_Write_NoCheck(pBuffer,WriteAddr,secremain);//写已经擦除了的,直接写入扇区剩余区间. 				   
...
{                                   
	...
	FLASH_CS=1;                            //取消片选     	      
	drv_FLASH_Wait_Busy();   				   	//等待芯片擦除结束
}   
//擦除一个扇区
//Dst_Addr:扇区地址 根据实际容量设置
//擦除一个山区的最少时间:150ms
void drv_FLASH_Erase_Sector(u32 Dst_Addr)   
{  
	//监视falsh擦除情况,测试用    	
 	Dst_Addr *= 4096;
    drv_FLASH_Write_Enable();                  //SET WEL 	 
	...
	FLASH_CS=1;                       		//取消片选     	      
    drv_FLASH_Wait_Busy();   				   	//等待擦除完成
}  
//等待空闲
void drv_FLASH_Wait_Busy(void)   
{   
	while((drv_FLASH_ReadSR()&0x01)==0x01);   // 等待BUSY位清空
}  
//进入掉电模式
void drv_FLASH_PowerDown(void)   
{ 
  	FLASH_CS=0;                            //使能器件   
